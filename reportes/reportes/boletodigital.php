...
$result=mysql_query("select ventas.costo_b as costob, ventas.comision as comision, ventas.iva_com as iva, ventas.costo as total, ventas.zona as zona, ventas.lugar as lugar,ventas.fila as fila, ventas.cuadrante as seccion, ventas.fecha_op as fecha, ventas.no_operacion as operacion, ventas.no_vta as folio,ventas.cva_lib as cry,ventas.imp as imp from ventas 
where  ventas.id=$id and status=1",$conexion);
...
$folio=$row["folio"];
$cry=$row["cry"];
$cliente="Punto de Venta Publico General";
$imp=$row['imp'];
}
...
SET ventas.imp = $imp 
WHERE ventas.id=$id",$conexion);

$fecha_imp=date('d M y h:m');
$artista="1F SASHA BENNY ERIK";
...
$pdf->TextWithRotation(360,400,$fila,360);
$pdf->TextWithRotation(430,400,$lugar,360);
$pdf->TextWithRotation(220,400,$zona,360);
$pdf->TextWithRotation(290,400,$seccion,360);
...
$pdf->SetFont('Arial','B',24);
 ...
 $pdf->SetFont('Arial','B',18);
 $pdf->TextWithRotation(100,300,'Fecha de Generacion',360);
 ...
 $pdf->TextWithRotation(280,550,$cry,360);
